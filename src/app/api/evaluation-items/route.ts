...
export async function POST(req: NextRequest) {
  ...
  for (const t of body as TargetInput[]) {
    if (typeof t.no !== "number" || typeof t.name !== "string" || !t.name.trim()) {
      return errorResponse("BAD_REQUEST", "大項目に no（数値）と name（文字列）は必須です", 400);
    }
    if (!Array.isArray(t.categories)) {
      return errorResponse("BAD_REQUEST", "大項目に categories 配列は必須です", 400);
    }
    for (const c of t.categories) {
      if (typeof c.no !== "number" || typeof c.name !== "string" || !c.name.trim()) {
        return errorResponse("BAD_REQUEST", "中項目に no（数値）と name（文字列）は必須です", 400);
      }
      if (!Array.isArray(c.items)) {
        return errorResponse("BAD_REQUEST", "中項目に items 配列は必須です", 400);
      }
      for (const item of c.items) {
        if (typeof item.no !== "number" || typeof item.name !== "string" || !item.name.trim()) {
          return errorResponse(
            "BAD_REQUEST",
            "評価項目に no（数値）と name（文字列）は必須です",
            400,
          );
        }
      }
