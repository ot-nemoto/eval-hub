...
export default async function AdminManagerEvaluationsPage({
  searchParams,
}: {
  ...
  const [rows, activeUsers] = await Promise.all([
    selectedYear !== null
      ? getAllManagerEvaluations(selectedYear, { userId: userId || undefined })
      : [],
  ...
  return (
    <div>
      <div className="mb-6">
        <h2 className="text-xl font-bold text-gray-900">全ユーザー上長評価一覧</h2>
        <p className="text-sm text-gray-500">年度・ユーザーで絞り込んで確認できます。</p>
      </div>

      <div className="mb-4 flex flex-wrap items-center gap-4">
        <AdminYearSelector
          basePath="/admin/manager-evaluations"
        ...
        <table className="w-full text-sm">
          <thead className="border-b bg-gray-50">
            <tr>
              ...
              <th className="px-4 py-3 text-left font-medium text-gray-700">上長採点</th>
              <th className="px-4 py-3 text-left font-medium text-gray-700">上長採点理由</th>
              <th className="px-4 py-3 text-left font-medium text-gray-700">評価者名</th>
                ...
                <tr key={r.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    {r.managerScore === null ? (
                      ...
                      >
                        {SCORE_LABEL[r.managerScore]}
                      </span>
                    )}
                  </td>
                  <td className="max-w-xs truncate px-4 py-3 text-gray-700">
                    {r.latestComment?.reason ?? <span className="text-gray-400">—</span>}
                  </td>
                  <td className="px-4 py-3 text-gray-700">
                    {r.latestComment?.evaluatorName ?? <span className="text-gray-400">—</span>}
                  </td>
