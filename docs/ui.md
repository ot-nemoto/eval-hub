...
| `ManagerEvaluationTabs` | Client Component | 評価者採点入力。中分類タブ・自己評価表示・最終スコアセクション（evaluations.manager_score、アサイン済み評価者/admin が上書き可）・コメントスレッド（投稿者名・スコア任意・理由・日時）・追加フォーム・自コメントの編集/削除 |
